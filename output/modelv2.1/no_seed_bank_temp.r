	...
	for(v in 1:length(vari_tmp)){
		...
		for(s in 1:nb_simu){
			...
			for(t in 1:(n_iter-1)){
				...
				#Without Seed Bank
                		var_tmp=step1(N_array[t,,,s,m,v],list_H,type_inter,temp_model[t],M,morta,a_d,T_opt,B)
  		              	Ntmp=var_tmp[[1]]
                		N_array[t+1,,,s,m,v]=step2(Ntmp,S,Gamma*(temp_model[t]>=temp_germin),e)
		
				#With Seed Bank
                		var_tmp=step1(N_orig[t,,,s,m,v],list_H,type_inter,temp_model[t],M_orig,morta,a_d,T_opt,B)
                		Ntmp=var_tmp[[1]]
