...
###CP 15/05/2020: Corrected the way average and amplitude were taken into account (before, was highly dependent on only the most abundant species)
#################
...
for(param_to_move in rownames(free_param)){
	...
	print(analyses[id_param])
		...
		print(paste(amp_sens["PRP"],"amp_sens"))
                tmp_amplitude=100*(amplitude_tot_original-amp_sens)/amplitude_tot_original
		print(paste(tmp_amplitude["PRP"],"tmp_amplitude"))
...
par(mfrow=c(1,2))
plot(0,0,t="n",xlim=c(0.5,nrow(free_param)+0.5),ylim=c(-6.5,4),xaxt="n",ylab="%Change in average abundance",xlab="")
axis(1,labels=rownames(free_param),at=1:nrow(free_param))
abline(h=0)
...
for(param_to_move in rownames(free_param)){
        ...
        for(i in 1:length(id_param)){
                ...
                if(tab_summary[id_param[i],4]==0){
                        points(l+seq_space[i],per_change+(0.1*10.5)*sign(per_change),t="p",pch=16,col="red")
                }
                at_val_text=c(at_val_text,l+seq_space[i])
                tmp_text=strsplit(analyses[id_param[i]],"_")
                val_text=c(val_text,tmp_text[[1]][length(tmp_text[[1]])])
...
plot(0,0,t="n",xlim=c(0.5,nrow(free_param)+0.5),ylim=c(-40,30),xaxt="n",ylab="%Change in average amplitude",xlab="")
axis(1,labels=rownames(free_param),at=1:nrow(free_param))
abline(h=0)
mtext(c(all_others),1,line=3,at=1:nrow(free_param),cex=0.8)
l=0
...
for(param_to_move in rownames(free_param)){
        ...
        for(i in 1:length(id_param)){
                ...
                rect(l+0.75*seq_space[i],min(c(0,per_change)),l+1.25*seq_space[i],max(c(per_change,0)),pch=16,col="grey")
                if(tab_summary[id_param[i],4]==0){
                        points(l+seq_space[i],per_change+(0.1*70)*sign(per_change),t="p",pch=16,col="red") #I add (0.1*70) to use the range of ylim and have, proportionally, the same height between rectangles and red dots in both panels
                }
                at_val_text=c(at_val_text,l+seq_space[i])
                tmp_text=strsplit(analyses[id_param[i]],"_")
                val_text=c(val_text,tmp_text[[1]][length(tmp_text[[1]])])
...
##If you want the same ylim for all species
...
for(s in seq(sp,sp+dim[d]-1)){
	...
	print(ydelim_mean)
        plot(0,0,t="n",xlim=c(0.5,nrow(free_param)+0.5),ylim=ydelim_mean+c(-1,1),xaxt="n",ylab="%Change in abundance",xlab="",main=dimnames(matrix_mean)[[3]][s]) #I add (-1,1) so that we can see the red dots
        if((s-sp+1)==dim[d]){
        axis(1,labels=rownames(free_param),at=1:nrow(free_param))
