...
doyouload=FALSE #If TRUE, load previous results to plot them ; if FALSE, relaunch simulations
...
save(N_array, N_orig, mean_val,min_val,range_val,fac_simu,nb_persistence,id,file = "no_seed_compet_simu_light_keeping_intra.RData", envir = .GlobalEnv)
}else{ #end !nodoyouload
load("no_seed_compet_simu_light_keeping_intra.RData")
}

id_persistence=seq(length(id)-nb_persistence+1,length(id)) 

pdf("no_seed_bank_compet_keeping_intra.pdf")
...
pdf("dynamics_vs_survival_no_seed_bank_minab_amplitude_niche_keeping_intra.pdf",width=9,height=3)
set.seed(42)
...
pdf("dynamics_vs_survival_no_seed_bank_meanab_sinking_temperature_keeping_intra.pdf",width=9,height=3)
set.seed(42)
...
text(min(c(T_opt))-0.1-diff(range(c(T_opt))+c(-0.1,0.5))*0.2,1.1,"c)",las=2,xpd=NA,font=2)
#points(B,surviv_facil[,1],col="blue",pch=16)
